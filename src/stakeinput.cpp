...
// The zCSM block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zCSM is a hash of the serial
...
{
    //Create an output returning the zCSM that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZCSMOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zCSM output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zCSM", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZCSMOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zCSM output", __func__);
...
{
    CzCSMTracker* zcsmTracker = pwallet->zcsmTracker.get();
    CMintMeta meta;
    if (!zcsmTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zcsmTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a CSM stake is the outpoint

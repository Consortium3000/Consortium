...
public:
    ...
    bool WriteZCSMSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCSMSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCSMSeed_deprecated(uint256& seed);
    bool EraseZCSMSeed();
    bool EraseZCSMSeed_deprecated();

    bool WriteZCSMCount(const uint32_t& nCount);
    bool ReadZCSMCount(uint32_t& nCount);

...
/**
 * Consortium RPC command dispatcher.
 */
class CRPCTable
{
private:
...
extern UniValue DoZcsmSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool isPublicSpend = true);
...
extern UniValue setzcsmseed(const UniValue& params, bool fHelp);
extern UniValue getzcsmseed(const UniValue& params, bool fHelp);
extern UniValue generatemintlist(const UniValue& params, bool fHelp);
extern UniValue searchdzcsm(const UniValue& params, bool fHelp);
extern UniValue dzcsmstate(const UniValue& params, bool fHelp);

...
{
    // The wallet only allows publicSpend inputs in the same tx and not a combination between csm and zcsm
    for(const CTxIn& txin : vin) {
...
CAmount CTransaction::GetValueOut() const
{
    ...
    {
        // Consortium: previously MoneyRange() was called here. This has been replaced with negative check and boundary wrap check.
        if (it->nValue < 0)

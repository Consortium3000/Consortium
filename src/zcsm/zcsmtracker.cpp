...
CzCSMTracker::CzCSMTracker(std::string strWalletFile)
{
...
CzCSMTracker::~CzCSMTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCSMTracker::Init()
{
...
bool CzCSMTracker::Archive(CMintMeta& meta)
{
...
bool CzCSMTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCSMTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCSMTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCSMTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzCSMTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzCSMTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzCSMTracker::GetSerialHashes()
{
...
CAmount CzCSMTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCSMTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCSMTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCSMTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCSMTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCSMTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCSMTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCSMTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCSMTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCSMTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCSMTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzCSMWallet* zCSMWallet)
{
    bool iszCSMWalletInitialized = (NULL != zCSMWallet);
    CMintMeta meta;
    ...
    if (! iszCSMWalletInitialized)
        zCSMWallet = new CzCSMWallet(strWalletFile);
    meta.isSeedCorrect = zCSMWallet->CheckSeed(dMint);
    if (! iszCSMWalletInitialized)
        delete zCSMWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzCSMTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCSMTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCSMTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCSMTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCSMTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCSMTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzCSMWallet* zCSMWallet = new CzCSMWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zCSMWallet);
        }
        delete zCSMWallet;
        LogPrint("zero", "%s: added %d dzcsm from DB\n", __func__, listDeterministicDB.size());
    }

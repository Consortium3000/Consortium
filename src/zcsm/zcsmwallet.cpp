...
CzCSMWallet::CzCSMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcsm seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCSMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCSMSeed_deprecated()) {
                    LogPrintf("%s: Updated zCSM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcsm seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcsm wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCSMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCSMCount(nCountLastUsed);
    else if (!walletdb.ReadZCSMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCSMWallet::Lock()
{
    seedMaster = 0;
}

void CzCSMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCSMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCSM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCSMWallet::LoadMintPoolFromDB()
{
...
void CzCSMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCSMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCSMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcsmTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCSMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCSM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcsmTracker which also adds to database
    pwalletMain->zcsmTracker->Add(dMint, true);
...
void CzCSMWallet::SeedToZCSM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCSMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCSMWallet::GenerateDeterministicZCSM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCSMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCSMWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzCSMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

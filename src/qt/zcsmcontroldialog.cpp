...
#include "ui_zcsmcontroldialog.h"
...
void ZPivControlDialog::updateList()
{
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        {
            LOCK(pwalletMain->zcsmTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zcsmTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zCSM.";
            else if (!mint.isSeedCorrect)
                strReason = "The zCSM seed used to mint this zCSM is not the same as currently hold in the wallet";
            else
